...
def check_login(source, username):
    ...
    if re.search(b'loggedin', source):
        return (True, None)
    return (False, None)
...
def doLogin(cookiepath, username, password):
    ...
    if username and password:
        ...
        login_url = 'https://secure.members.easynews.com/login/'
        ...
        login_data = urllib_parse.urlencode({'username': username,
                                             'password': password,
                                             'do': 'login',
                                             'SSO': ''
                                             })
        ...
        cj = http_cookiejar.MozillaCookieJar(cookiepath)
        ...
        try:
            resp = urllib_request.urlopen(req, login_data.encode())
            source = resp.read()
        ...
        if login:
            cj.save()
            xbmc.log('cookies: %s' % cj._cookies)
